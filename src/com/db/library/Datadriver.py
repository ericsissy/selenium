...
Purpose: To cycle the excel and get certain cell's values, put them into
        input box gradually.
        Usually its used for register in batch, but sometimes we use it 
        to login system, in this situation, we should have a status of login
        result in excel for good.
'''
...
class ExcelSheet:
    def __init__(self, excel, sheet):
        self.excel = xlrd.open_workbook(r"%s\pre_data\%s" % (Conf.PROJECT_PATH, excel))
