...
class Element(WebElement):
    ...
    def find_elements(cls, by, value=None):
        ...
        return elename_name
#     @classmethod
#     def find_elements(cls, driver, by, value=None):
#         '''Return the elements by find_elements()
#         '''
#         try:
#             elename_name = driver.find_elements(by, value)
#         except NoSuchElementException:
#             Log.handle_error(driver)
#             
#         return elename_name
#     
#     @classmethod
...
#         return [k.text for k in elename_name]
    ...
    @classmethod
    def date_picker(self, day, month, year):
        """
        To pick a certain day as expected
        This function will not handle leap month
        
        :Args:
         - day:        certain day like : 15
         - month:      certain month like : 8
         - year:       certain year like : 2013
        
        :Usage:
        - To be invoked in page package : 
            Element.date_picker(31, 3, 2014)
        - and will be used in a certain case : 
            switch_to_window(get_windows()[-1])
            usePage("OverviewSearchDatePicker")
            switch_to_window(get_windows()[0])
            
        """
        months = {"january":1, "february":2, "march":3, "april":4, "may":5, "june":6, \
                  "july":7, "august":8, "september":9, "october":10, "november":11, "december":12}
        days = '//font[@color="#000000"][text()=' + str(day) + ']'
        date_picker_header = 'html/body/table/tbody/tr/td/table/tbody/tr[1]/td/table/tbody/tr/td[2]/font'
        date_picker_pre_month = '//img[@alt="previous month"]'
        date_picker_next_month = '//img[@alt="next month"]'
        date_picker_pre_year = '//img[@alt="previous year"]'
        date_picker_next_year= '//img[@alt="next year"]'
        
        POFS_DATE_MM_YY = Conf.DRIVER.find_element(By.XPATH, date_picker_header)
        def_month = months[POFS_DATE_MM_YY.text[:-5].lower()]
        if def_month == month:
            pass
        elif def_month > month:
            month_interval = def_month - month
            while month_interval > 0:
                Conf.DRIVER.find_element(By.XPATH, date_picker_pre_month).click()
                month_interval -= 1
                time.sleep(1)
        elif def_month < month:
            month_interval = month - def_month
            while month_interval > 0:
                Conf.DRIVER.find_element(By.XPATH, date_picker_next_month).click()
                month_interval -= 1
                time.sleep(1)
        else:
            print "Error Date"
        ...
        POFS_DATE_MM_YY = Conf.DRIVER.find_element(By.XPATH, date_picker_header)
        def_year = int(POFS_DATE_MM_YY.text[-5:])
        if def_year == year:
            Conf.DRIVER.find_element(By.XPATH, days).click()
        elif def_year > year:
            year_interval = def_year - year
            while year_interval > 0:
                Conf.DRIVER.find_element(By.XPATH, date_picker_pre_year).click()
                year_interval -= 1
                time.sleep(1)
            Conf.DRIVER.find_element(By.XPATH, days).click()
        elif def_year < year:
            year_interval = def_year - year
            while year_interval < 0:
                Conf.DRIVER.find_element(By.XPATH, date_picker_next_year).click()
                year_interval -= 1
                time.sleep(1)
            Conf.DRIVER.find_element(By.XPATH, days).click()
        else:
